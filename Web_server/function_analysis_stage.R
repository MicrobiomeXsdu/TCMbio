...
# # #"ajcc_pathologic_2_stage"
# # #"clinical_stage"         
# # #"clinical_2_stage" 
# group <- "ajcc_pathologic_2_stage"
# 
# # ##sub参数可选"I" "II" "III" "IV" NULL
# sub1 <- NULL
# sub2 <- NULL
# plot.width <- 5
# plot.height <- 4
...
if (group == "clinical_2_stage"){
  data_sub2 <- data_sub
  data_sub2$clinical_2_stage <- as.factor(data_sub2$clinical_2_stage)
  data_sub2 <- subset(data_sub2,data_sub2$SampleType=="1")
  data_sub2 <- data_sub2[,!colnames(data_sub2) %in% "SampleType"]
  data_sub2 <- data_sub2[which(!is.na(data_sub2$clinical_2_stage)),]
  if (nrow(data_sub2)==0 | length(unique(data_sub2$clinical_2_stage))==1) {
    ...
    for (i in 1:(length(colnames(data_sub2))-1)) {
      res_result <- wilcox.test(data_sub2[,i]~data_sub2$clinical_2_stage)
    ...
    bar_data <-   aggregate(data_sub2[,1:length(colnames(data_sub2))-1], by = list(data_sub2$clinical_2_stage),FUN=mean)
  ...
  if (group == "clinical_stage"){
    data_sub2 <- data_sub
    data_sub2$clinical_stage <- as.factor(data_sub2$clinical_stage)
    data_sub2 <- subset(data_sub2,data_sub2$SampleType=="1")
    data_sub2 <- data_sub2[,!colnames(data_sub2) %in% "SampleType"]
    data_sub2 <- subset(data_sub2,data_sub2$clinical_stage==sub1 | data_sub2$clinical_stage==sub2)
    if (nrow(data_sub2)==0 | length(unique(data_sub2$clinical_stage))==1) {
      print("Under this condition, the sample size is insufficient.")
    } else {
      print("Under this condition, the sample size is sufficient.")
      result_table <- matrix(ncol = 3,nrow = length(colnames(data_sub2))-1)
      ...
      result_table$name <- colnames(data_sub2)[1:length(colnames(data_sub2))-1]
      for (i in 1:(length(colnames(data_sub2))-1)) {
        res_result <- wilcox.test(data_sub2[,i]~data_sub2$clinical_stage)
        result_table[i,2] <- res_result[["p.value"]]
        if (Download_single_plot == "Yes") {
          if(res_result[["p.value"]]<p_value){
            pic <- ggplot(data_sub2, aes(x=cancer_type,
      ...
      #写出list
      write.csv(result_table,file = paste0("result/function_result_",cancer_type,"_",type,"_",group,"_",sub1,"_",sub2,"_table_result.csv"))
      bar_data <-   aggregate(data_sub2[,1:length(colnames(data_sub2))-1], by = list(data_sub2$clinical_stage),FUN=mean)
      colnames(bar_data)[1] <- "group"
      rownames(bar_data) <- bar_data$group
      bar_data_sig <- bar_data[,colnames(bar_data) %in% result_table$name[which(result_table$adj_p_value<p_value)]]
      bar_data_sig <- data.frame(bar_data_sig)
    ...
    if(group == "ajcc_pathologic_2_stage"){
      data_sub2 <- data_sub
      data_sub2$ajcc_pathologic_2_stage <- as.factor(data_sub2$ajcc_pathologic_2_stage)
      data_sub2 <- subset(data_sub2,data_sub2$SampleType=="1")
      data_sub2 <- data_sub2[,!colnames(data_sub2) %in% "SampleType"]
      data_sub2 <- data_sub2[which(!is.na(data_sub2$ajcc_pathologic_2_stage)),]
      if (nrow(data_sub2)==0 | length(unique(data_sub2$ajcc_pathologic_2_stage))==1) {
        print("Under this condition, the sample size is insufficient.")
      } else {
        print("Under this condition, the sample size is sufficient.")
        result_table <- matrix(ncol = 3,nrow = length(colnames(data_sub2))-1)
        ...
        result_table$name <- colnames(data_sub2)[1:length(colnames(data_sub2))-1]
        for (i in 1:(length(colnames(data_sub2))-1)) {
          res_result <- wilcox.test(data_sub2[,i]~data_sub2$ajcc_pathologic_2_stage)
          result_table[i,2] <- res_result[["p.value"]]
          if (Download_single_plot == "Yes") {
            if(res_result[["p.value"]]<p_value){
              pic <- ggplot(data_sub2, aes(x=cancer_type,
        ...
        #写出list
        write.csv(result_table,file = paste0("result/function_result_",cancer_type,"_",type,"_",group,"_",sub1,"_",sub2,"_table_result.csv"))
        bar_data <-   aggregate(data_sub2[,1:length(colnames(data_sub2))-1], by = list(data_sub2$ajcc_pathologic_2_stage),FUN=mean)
        colnames(bar_data)[1] <- "group"
        rownames(bar_data) <- bar_data$group
        bar_data_sig <- bar_data[,colnames(bar_data) %in% result_table$name[which(result_table$adj_p_value<p_value)]]
        bar_data_sig <- data.frame(bar_data_sig)
      ...
      if (group == "ajcc_pathologic_stage") {
        data_sub2 <- data_sub
        data_sub2$ajcc_pathologic_stage <- as.factor(data_sub2$ajcc_pathologic_stage)
        data_sub2 <- subset(data_sub2,data_sub2$SampleType=="1")
        data_sub2 <- data_sub2[,!colnames(data_sub2) %in% "SampleType"]
        data_sub2 <- subset(data_sub2,data_sub2$ajcc_pathologic_stage==sub1 | data_sub2$ajcc_pathologic_stage==sub2)
        if (nrow(data_sub2)==0 | length(unique(data_sub2$ajcc_pathologic_stage))==1) {
          print("Under this condition, the sample size is insufficient.")
        } else {
          print("Under this condition, the sample size is sufficient.")
          result_table <- matrix(ncol = 3,nrow = length(colnames(data_sub2))-1)
          ...
          result_table$name <- colnames(data_sub2)[1:length(colnames(data_sub2))-1]
          for (i in 1:(length(colnames(data_sub2))-1)) {
            res_result <- wilcox.test(data_sub2[,i]~data_sub2$ajcc_pathologic_stage)
            result_table[i,2] <- res_result[["p.value"]]
            if (Download_single_plot == "Yes") {
              if(res_result[["p.value"]]<p_value){
                pic <- ggplot(data_sub2, aes(x=cancer_type,
          ...
          #写出list
          write.csv(result_table,file = paste0("result/function_result_",cancer_type,"_",type,"_",group,"_",sub1,"_",sub2,"_table_result.csv"))
          bar_data <-   aggregate(data_sub2[,1:length(colnames(data_sub2))-1], by = list(data_sub2$ajcc_pathologic_stage),FUN=mean)
          colnames(bar_data)[1] <- "group"
          rownames(bar_data) <- bar_data$group
          bar_data_sig <- bar_data[,colnames(bar_data) %in% result_table$name[which(result_table$adj_p_value<p_value)]]
          bar_data_sig <- data.frame(bar_data_sig)
...
# Rscript function_analysis_stage.R LUSC pathway 0.30 ajcc_pathologic_2_stage NULL NULL 5 4 Fungus No
