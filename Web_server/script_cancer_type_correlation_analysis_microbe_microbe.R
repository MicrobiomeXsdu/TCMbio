...
if (!is.null(taxab)) {
  x=as.numeric(otu[,taxaa])
  y=as.numeric(otu[,taxab])
  ...
  outTab=data.frame()
  outVector=cbind(Xlab=taxaa, Ylab=taxab, cor=cor$estimate, pvalue=cor$p.value)
  outTab=rbind(outTab,outVector)
  outFile=paste0("result/cancer_type_correlation_analysis_",cancer_type,"_",taxaa,"_cor_", taxab,"_",correlation_coef,".pdf")
  df1=as.data.frame(cbind(x,y))
  p1=ggplot(df1, aes(x, y)) + 
    xlab(paste0(taxaa)) + ylab(taxab)+
  ...
  write.table(outTab,file=paste0("result/temp/",cancer_type,"_",taxaa,"_",taxab,"_",correlation_coef,"_cor_","result.txt"),sep="\t",row.names=F,quote=F)
} else {
  print("error")
  q()
}

# Rscript script_cancer_type_correlation_analysis_microbe_microbe.R 'LUSC' 'g__Yarrowia' 'g__Penicillium' 'pearson' '6' '6' 'Fungus'